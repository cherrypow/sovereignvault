...
/// Escalating lockout schedule for passphrase/master-password attempts:
///   10 wrong in a row  -> 24 hour lockout
///   10 more wrong      -> 72 hour lockout
///   10 more wrong      -> crypto-shred (vault destroyed)
///
/// There is no way to clear an active lockout or reset the stage early
/// — not even with the correct master password. See KeyManager: both
/// unlock paths check [checkStatus] before touching key material, and
/// neither ever calls a "clear" method, because none exists here.
///
...
/// attempting passphrases and restores that image after each lockout
/// can reset this counter indefinitely. Making this trustworthy against
...
class LockoutPolicy {
  ...
  /// unlock with either the passphrase or the master password.
  static Future<LockoutState> checkStatus() => _load();

  /// Records a failed unlock attempt and returns the resulting state,
  ...
  /// Called on a successful unlock (passphrase or master password) —
  /// resets the failed-attempt counter but deliberately leaves [stage]
  /// untouched. The stage never resets, by anything, short of the final
  /// wipe: a series of successful unlocks between lockouts must not
  /// quietly reset an attacker's progress toward being locked out, and
  /// there is no secret strong enough to be trusted to reset it either.
